...
public class Loader {
	public static String load(MachineModel model, File program, File data) {
		if (model == null || program == null || data == null)
			return null;

		try (Scanner input = new Scanner(program)) {
			// while loop to process program
			...
			try (Scanner input2 = new Scanner(data)) {
	...
	public static void main(String[] args) {
		...
		try (Scanner keyboard = new Scanner(System.in)) {
			...
			while (test.isRunning()) {
				...
				test.step();
				// the following lines mess up the behavior for positive numbers
				// but
